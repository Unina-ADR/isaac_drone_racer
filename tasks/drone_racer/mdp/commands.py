...
class GateTargetingCommand(CommandTerm):
    ...
    def __init__(self, cfg: GateTargetingCommandCfg, env: ManagerBasedEnv):
        ...
        self.gate_after= torch.zeros(self.num_envs, 7, device=self.device)
        self.gate_after_idx = torch.zeros(self.num_envs, dtype=torch.int32, device=self.device)
        self._point_to_next_gate = torch.zeros(self.num_envs, 3, device=self.device)
    ...
    @property
    def next_command(self) -> torch.Tensor:
        """position of the gate after the current one."""
        return self.gate_after

    @property
    def command_idx(self) -> torch.Tensor:
        """The index of the next gate for each environment."""
        return self.next_gate_idx

    @property
    def num_commands(self) -> int:
        """The number of commands generated by this command generator."""
    ...
    @property
    def point_to_next_gate(self) -> torch.Tensor:
        """The unit vector pointing to the next gate."""
        return self._point_to_next_gate
    ...
    def _resample_command(self, env_ids: Sequence[int]):
        ...
        if self.cfg.randomise_start is None:
            ...
            if self.cfg.randomise_start:
                ...
                self.gate_after_idx[env_ids] = (self.next_gate_idx[env_ids] + 1) % self.num_gates
            else:
                self.next_gate_idx[env_ids] = 1
                self.gate_after_idx[env_ids] = 2
    ...
    def _update_command(self):
        ...
        #logic to pass second waypoint to obs manaer
        gate_after_positions = self.track.data.object_com_pos_w[self.env_ids, self.gate_after_idx]
        gate_after_orientations = self.track.data.object_quat_w[self.env_ids, self.gate_after_idx]
        self.gate_after = torch.cat([gate_after_positions, gate_after_orientations], dim=1)

        # compute the vector pointing to the next gate
        seg_to_next_gate = self.gate_after[:, :3] - self.next_gate_w[:, :3]
        self._point_to_next_gate = seg_to_next_gate / torch.norm(seg_to_next_gate, dim=1, keepdim=True)
        ...
        self.gate_after_idx[self._gate_passed] += 1
        self.gate_after_idx = self.gate_after_idx % self.num_gates
