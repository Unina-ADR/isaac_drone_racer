...
from dynamics import Allocation  # isort: skip
...
class HoverEnvCfg(DirectRLEnvCfg):
    ...
    omega_max: float = 1855.0
    """Maximum rotor angular speed in rad/s. Corresponds to motor command = 1. """
    thrust_coef: float = 1.52117969e-5
    """Thrust coefficient in N/(rad/s)^2. For the A2R drone"""
    drag_coef: float = 7.6909678e-9
    """Drag (torque) coefficient in Nm/(rad/s)^2. For the A2R drone"""
    ...
    """Whether to use a motor dynamics model."""
...
class HoverEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: HoverEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        self._allocation = Allocation(
            ...
            device=self.device,
            #dtype=self.cfg.thrust_coef.dtype,
        )
        self._motor = Motor(
            num_envs=self.num_envs,
            taus=self.cfg.taus,
    ...
    def _pre_physics_step(self, actions: torch.Tensor):
        self._actions[:] = actions #.clamp_(-1.0, 1.0)
        clamped = self._actions.clamp_(-1.0, 1.0)
